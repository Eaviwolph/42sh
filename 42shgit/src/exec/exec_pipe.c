...
*/
static pid_t exec_left_hand(int p[2], s_ast_node *left, s_pipeline *pipeline);
/*!
...
** @return pid for father, exit with last_status for the son
*/
static pid_t exec_right_hand(int p[2], s_ast_node *right, s_pipeline *pipeline);
...
static void push_pid(s_pipeline *pipeline, pid_t new)
{
    ...
    if (new < 0)
        return;
    secrealloc(pipeline->list, pipeline->list,
