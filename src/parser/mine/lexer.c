...
#include "../../common/mine/function.h"
#include "../../common/mine/macro.h"
#include "../../shell/mine/shell.h"
#include "getline.h"
#include "parser.h"
...
** - Operators are chars which used to cut token and which are returned as
*token.
...
static const s_token operators[] = {
    { TOK_NEWLINE, "\n", 1 }, { TOK_AND, "&&", 2 },
    { TOK_SEPAND, "&", 1 },   { TOK_OR, "||", 2 },
    { TOK_PIPE, "|", 1 },     { TOK_DSEMI, ";;", 2 },
    { TOK_SEP, ";", 1 },      { TOK_DLESSDASH, "<<-", 3 },
    { TOK_DLESS, "<<", 2 },   { TOK_LESSGREAT, "<>", 2 },
    { TOK_LESSAND, "<&", 2 }, { TOK_LESS, "<", 1 },
    { TOK_DGREAT, ">>", 2 },  { TOK_GREATAND, ">&", 2 },
    { TOK_CLOBBER, ">|", 2 }, { TOK_GREAT, ">", 1 },
    { TOK_LPAREN, "(", 1 },   { TOK_RPAREN, ")", 1 },
    { TOK_NONE, NULL, 0 }
};
...
** @param buf_pos position in the buffer, which is incremented if found to point
*on
...
** @return true (!0) if a quote is found, else false (0)
*/
static int is_quote_stop(const char *buf, size_t *buf_pos,
                         const s_quote *quote);
...
static void lexer_eattoken(s_lexer *lexer)
{
    ...
    if (!lexer->buf_size && !lexer->eof
        && (lexer->buf = getline(lexer->stream)))
    {
