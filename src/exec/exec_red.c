...
#define is_open_read(flags)                                                    \
    (((flags)&0x3) == O_RDONLY || ((flags)&0x2) == O_RDWR)

#define is_open_write(flags)                                                   \
    (((flags)&0x3) == O_WRONLY || ((flags)&0x2) == O_RDWR)
...
static int apply_red(s_red_node *reds)
{
    ...
    for (size_t i = 0; i < reds->size; ++i)
    {
        ...
        case R_CLOBBER: // >|
            if ((fd = open(reds->word[i], O_WRONLY | O_CREAT | O_TRUNC, 0666))
                == -1)
                rederror(reds->word[i], strerror(errno));
            dup2(fd, reds->fd[i]);
            close(fd);
            break;
        case R_DGREAT: // >>
            if ((fd = open(reds->word[i], O_WRONLY | O_CREAT | O_APPEND, 0666))
                == -1)
                rederror(reds->word[i], strerror(errno));
            dup2(fd, reds->fd[i]);
            close(fd);
        ...
        case R_DLESS: // <<
            assert(0);
            /*       if ((fd = mkstemp(filename =
             * strdup("42sh.heredocs.XXXXXX"))) == -1) */
            /* 	rederror("42sh"); */
